...
class NotificationService {
  ...
  async initialize() {
    ...
    try {
      ...
      const { error } = await supabase.from('user_notifications').select('count').limit(1);
      if (error) {
  ...
  async create({
    ...
    try {
      ...
      if (this.isDuplicate(userId, category, metadata.productId)) {
        console.log('🔄 Duplicate notification prevented:', { userId, category, productId: metadata.productId });
        return null;
      }
  ...
  cleanupDeduplicationCache() {
    ...
    console.log(`🧹 Cleaned ${entriesToDelete.length} old deduplication entries`);
  }
   ...
   */
  async notifyExpiringSoon(productId, productName, expiryDate, daysRemaining, userId) {
    const isCritical = daysRemaining <= 7;
    
    return await this.create({
      userId,
      title: isCritical ? '🚨 Urgent: Product Expiring Soon' : '📅 Product Expiry Warning',
      message: `${productName} expires in ${daysRemaining} day${daysRemaining === 1 ? '' : 's'} (${expiryDate})`,
  ...
  async notifySaleCompleted(saleId, totalAmount, itemCount, userId) {
    return await this.create({
      ...
      message: `Successfully processed sale of ${itemCount} item${itemCount === 1 ? '' : 's'} for ₱${totalAmount.toFixed(2)}`,
  ...
  async sendEmailNotification(notification) {
    try {
      ...
      if (emailResult.success) {
        ...
        console.warn('⚠️ Email send failed:', emailResult.error || emailResult.reason);
      }
  ...
  generateEmailTemplate(notification, userName) {
    const priorityColor = notification.priority <= NOTIFICATION_PRIORITY.HIGH ? '#dc2626' : '#f59e0b';
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="notification-box">
              <span class="priority-badge">${notification.priority <= NOTIFICATION_PRIORITY.HIGH ? 'URGENT' : 'Important'}</span>
   ...
   */
  async getUserNotifications(userId, { limit = 50, offset = 0, unreadOnly = false, category } = {}) {
    try {
      let query = supabase
        .from('user_notifications')
        .select('*', { count: 'exact' })
  ...
  subscribeToNotifications(userId, onNotificationUpdate) {
    ...
    this.realtimeSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('📬 Real-time notification update:', payload.eventType, payload.new?.title);
  ...
  async runHealthChecks() {
    try {
      ...
      console.log(`✅ Health checks completed: ${lowStockCount} low stock, ${expiringCount} expiring products`);
      ...
      try {
        ...
        console.error('❌ Failed to notify admin about health check failure:', notifyError);
      }
    }
  }
