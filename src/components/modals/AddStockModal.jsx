...
const AddStockModal = ({ isOpen, onClose, product, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('🔄 ProductService loaded:', ProductService);
      
      const result = await ProductService.addProductBatch(batchData);
      
      console.log('✅ Full batch result:', result);
      
      if (result && result.success) {
        console.log('✅ Batch added successfully with ID:', result.batch_id);
        onSuccess && onSuccess({ success: true, batchId: result.batch_id, data: batchData, result });
        onClose();
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
              ...
              <p className="text-xs text-green-600 mt-1">
                Format: BT + Date (YYMMDD) + Unique ID (e.g., BT250924-123)
              </p>
            </div>
          ...
          <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-blue-700 text-xs">
              ...
              The system ensures every batch has a unique, meaningful identifier.
            </p>
          </div>
        </div>
      </div>
