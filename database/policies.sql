...
CREATE POLICY "users_can_update_own_profile" ON users
    ...
    WITH CHECK (
        auth.uid() = id
        -- Note: Role and active status changes are restricted by separate policies
    );

-- Admin can soft delete users (set inactive)
CREATE POLICY "admins_can_deactivate_users" ON users
    FOR UPDATE
    USING (
        EXISTS (
            SELECT 1 FROM users 
...
-- Only admin can delete products (soft delete by setting inactive)
CREATE POLICY "admins_can_deactivate_products" ON products
    FOR UPDATE
    USING (
        EXISTS (
            SELECT 1 FROM users 
...
-- Users can update their own sales (within limits)
CREATE POLICY "users_can_update_own_sales" ON sales
    FOR UPDATE
    USING (
        user_id = auth.uid()
        AND status = 'pending'  -- Only pending sales can be updated
    )
    WITH CHECK (
        user_id = auth.uid()
        -- Note: user_id cannot be changed - enforced by database constraints
    );
...
-- Only admin can delete sale items from completed sales
CREATE POLICY "admins_can_delete_completed_sale_items" ON sale_items
    FOR DELETE
    USING (
        EXISTS (
            SELECT 1 FROM users 
...
-- Only admin can update stock movements (for corrections)
...
-- Only admin can delete stock movements (rare, for data cleanup)
...
-- The product_stock_status and sales_summary views will automatically
-- respect the RLS policies on the products, sales, and users tables
...
-- ==========================================
-- 9. ADDITIONAL SECURITY CONSTRAINTS
-- ==========================================
...
CREATE INDEX idx_users_auth_lookup ON users(id, role, is_active) WHERE is_active = true;

-- Partial indexes for active records
CREATE INDEX idx_active_users ON users(role) WHERE is_active = true;
CREATE INDEX idx_active_products ON products(category) WHERE is_active = true;
...
-- Grant read access to anonymous users for public endpoints (if needed)
-- GRANT SELECT ON products TO anon; -- Uncomment if needed for public catalog

COMMENT ON TABLE audit_log IS 'Audit trail for all sensitive database operations';

-- RLS policies and security functions ready! 🔒
